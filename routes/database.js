...
exports.cb_prev_ranking = function(pool, data, next_callback) {
    sql_query =   "select name, " +
                    "(@rank := @rank + 1) AS rank " +
                    "from " +
                    "(select name, avg(score) as score from account a, record b where a.id = b.id and regdate < '2017-08-27' and regdate < '2017-09-11' group by a.id order by avg(score) desc) as a, " +
